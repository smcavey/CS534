...
def experiment(n, df, niters,seed = 1):
    '''
    ...
    for i in range(niters):
        ...
        cost = run_Astar(board, fringe_chess,found_list,start_time)
        if cost != -1:
            temp = pd.DataFrame([[board, cost]], columns=['board','cost'])
...
if __name__ == '__main__':
    ...
    df = pd.DataFrame()
    df = experiment(4, df, 5000)
    df = experiment(5, df, 4000)
    df = experiment(6, df, 500)
    df = experiment(7, df, 200)
    df = experiment(8, df, 100)
    df.to_csv('board_scores.csv')
